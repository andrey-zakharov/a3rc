...
for [{_i=0},{_i<=2},{_i=_i+1}] do {
	...
	if((_usable_channels_id select _ch)>=0) then //_ch in _usable_channels == true
	{
		...
		if(_channel_array select _ch) then {
			ctrlSetText[(IDC_CHECKBOXES_BACKGROUND select _i),guiChBoxAct ];		
		}else{
			if((_active_lines==get_TM_MAXPORTS(0))) then {
			// All station lines are taken
			ctrlSetText[(IDC_CHECKBOXES_BACKGROUND select _i),guiChBoxOff]; 
			}else{
			// There is at least on station line still available
			ctrlSetText[(IDC_CHECKBOXES_BACKGROUND select _i),guiChBoxInAct];
			}
		};
	}else{ // will only be needed when max_chat_lines less than 3 need to rework that
...
if(_mode) then {
	with uiNamespace do 
	{
		...
		for [{_i=0},{_i<_usable_ch_count},{_i=_i+1}] do 
		{
			...
			{
			control ctrlSetTextColor GreyOn;
			}else{
