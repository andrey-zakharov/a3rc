...
/*Trigger Functions*/
TriggerDeactivateStation=Fnc_Prepare("VON_PLUS_GUI\Resources\VPG_trigger_DectivateStation.SQF");
/*TMST Plug-in*/
TMST_INIT =           Fnc_Prepare("VON_PLUS_GUI\init_stations.SQF");

/*GUI*/
GUI_toggleCH =        Fnc_Prepare("VON_PLUS_GUI\Resources\GUI_fnc_ToggleChannel.SQF");
GUI_displayedCH=      Fnc_Prepare("VON_PLUS_GUI\Resources\GUI_fnc_getDisplayedChannels.SQF");
GUI_refresh =         Fnc_Prepare("VON_PLUS_GUI\Resources\GUI_fnc_UpdateDisplay.SQF");
GUI_Jump2CH =         Fnc_Prepare("VON_PLUS_GUI\Resources\GUI_fnc_Jump2Channel.SQF");
GUI_RollCH =          Fnc_Prepare("VON_PLUS_GUI\Resources\GUI_fnc_RollChannel.SQF");
GUI_changeRollSpeed = Fnc_Prepare("VON_PLUS_GUI\Resources\GUI_fnc_ChangeRollSpeed.SQF");
...
if (!isDedicated) then { //On client side if machine is not an dedicated server
	...
	if(checkRadio) then{
	VPG_MENUE_ACTION = player addAction ["Open Radio Menu","VON_PLUS_GUI\Resources\Display_GUI.SQF","",1,true,true,"","(""ItemRadio"" in assignedItems player)"]; //Dialog only available when having a radio!		
	player addEventHandler ["Respawn",{VPG_MENUE_ACTION = player addAction ["Open Radio Menu","VON_PLUS_GUI\Resources\Display_GUI.SQF","",1,true,true,"","(""ItemRadio"" in assignedItems player)"];_nil=[] spawn PlayerReActivateAllCH}];
	player addEventHandler ["Put", "if(""ItemRadio"" in _this)then {[] spawn PlayerRemoveAllCH ;};"]; //dropping Radio means loosing all channels!
	}else{
	VPG_MENUE_ACTION = player addAction ["Open Radio Menu","VON_PLUS_GUI\Resources\LoadGUI.SQF"];
	player addEventHandler ["Respawn",{VPG_MENUE_ACTION = player addAction["Open Radio Menu","VON_PLUS_GUI\Resources\Display_GUI.SQF"]}];
